...
class JunitReporter extends WDIOReporter {
    ...
    private _buildOrderedReport(builder: JUnitReportBuilder, runner: RunnerStats, specFileName: string) {
        const rootSuites: SuiteStats[] = []
        ...
        for (const suiteKey of Object.keys(this.suites)) {
            ...
            for (const testKey of Object.keys(suite.tests)) {
                ...
                const rootTestCaseIndex = rootTestCases.findIndex((testcase: any) => testcase._attributes.name.includes(suiteNameWithoutFormat))
                    ...
                    if (test.error) {
                        ...
                        if (this.options.errorOptions) {
                            ...
                            for (const key of Object.keys(errorOptions)) {
                                rootTestCase[key]((test.error as any)[errorOptions[key]])
                            }
                        } else {
                            // default
